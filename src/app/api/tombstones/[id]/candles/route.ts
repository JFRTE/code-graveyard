...
export async function POST(
  ...
  // Rate limit
  const rateLimit = await checkRateLimit(`candle:${session.user.id}`, 10, 60)
  ...
  // Atomic increment candle count
  // Atomic increment (best effort - falls back gracefully)
  try {
    64|  await supabase.rpc('increment_counter', { row_id: params.id, column_name: 'candle_count' })
  } catch (_) {}
