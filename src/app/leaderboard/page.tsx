    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">{t.leaderboard.subtitle}</p>
        </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                {t.leaderboard.flowers}
              </h2>
              <div className="space-y-3">
                {topFlowers.map((item, i) => (
                  <Link key={item.id} href={`/tombstone/${item.id}`}>
                    <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-lg hover:border-purple-500/50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 dark:text-white truncate">{item.code_name}</p>
                      ...
                      <div className="flex items-center gap-1 text-pink-600 dark:text-pink-400">
                        ...
                        <span className="font-bold">{item.flower_count}</span>
                      </div>
                    </div>
                  </Link>
                ))}
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                {t.leaderboard.eulogies}
              </h2>
              <div className="space-y-3">
                {topEulogies.map((item, i) => (
                  <Link key={item.id} href={`/tombstone/${item.id}`}>
                    <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-lg hover:border-purple-500/50 transition-colors">
                      <span className="text-2xl w-10 text-center">{medals[i] || `#${i + 1}`}</span>
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 text-blue-600 dark:text-blue-400">
                        ...
                        <span className="font-bold">{item.eulogy_count}</span>
                      </div>
                    </div>
                  </Link>
                ))}
