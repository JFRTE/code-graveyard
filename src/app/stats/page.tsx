...
import { BarChart3, Skull, Flower2, MessageSquare, Calendar, TrendingUp } from 'lucide-react'
...
export default function StatsPage() {
  ...
  const fetchAll = async () => {
    try {
      const res = await fetch('/api/tombstones?limit=10000')
  ...
  const pieData = Object.entries(causeCounts)
    .map(([cause, count], i) => {
      const info = CAUSE_OF_DEATH_LABELS[cause as keyof typeof CAUSE_OF_DEATH_LABELS]
      return {
        label: info?.label || cause,
        value: count,
  ...
  const maxLangCount = langData[0]?.count || 1
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const count = tombstones.filter(t => {
  ...
  // Totals
  const totalFlowers = tombstones.reduce((s, t) => s + t.flower_count, 0)
  const totalEulogies = tombstones.reduce((s, t) => s + t.eulogy_count, 0)
  const totalCandles = tombstones.reduce((s, t) => s + (t.candle_count || 0), 0)
    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {[
            { icon: Skull, label: '总墓碑', value: tombstones.length, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' },
            { icon: Flower2, label: '总献花', value: totalFlowers, color: 'text-pink-600 dark:text-pink-400', bg: 'bg-pink-50 dark:bg-pink-900/20' },
            { icon: MessageSquare, label: '总悼词', value: totalEulogies, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' },
            { icon: Calendar, label: '总蜡烛', value: totalCandles, color: 'text-yellow-600 dark:text-yellow-400', bg: 'bg-yellow-50 dark:bg-yellow-900/20' },
          ].map((card, i) => (
            <motion.div
              key={i}
        ...
        <motion.div
          ...
          className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl p-8 mb-8"
        >
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            <Skull className="w-5 h-5 text-purple-600 dark:text-purple-400" />
            死因分布
          </h2>
          {pieData.length > 0 ? (
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 space-y-3">
                {pieData.map((d, i) => {
                  const pct = ((d.value / tombstones.length) * 100).toFixed(1)
                  return (
              ...
              {langData.map((d, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-6 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${(d.count / maxLangCount) * 100}%` }}
          ...
          <div className="flex items-end justify-between gap-2 h-40">
            {monthlyData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs text-gray-500 dark:text-gray-400 font-mono">{d.count}</span>
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: `${Math.max((d.count / maxMonthly) * 120, 4)}px` }}
                  transition={{ duration: 0.5, delay: i * 0.1 }}
                  className="w-full rounded-t-lg bg-gradient-to-t from-purple-600 to-indigo-400 min-h-[4px]"
                />
